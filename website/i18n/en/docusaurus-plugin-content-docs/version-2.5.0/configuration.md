...
Settings changed by write operations return to their default values when the cube is disconnected.
For settings that are retained even after the cube is turned off and on again, see [Persistent configuration](persistent_configuration.md).
...
### Serialized information notification settings

Write the following data to enable or disable the [Serialized information](serialized_information.md) function. This function is disabled by default. See [Responses to serialized information notification settings](#responses-to-serialized-information-notification-settings) for how to obtain setting results.

| Data location | Type  | Content                | Example                                                                     |
| ------------- | ----- | ---------------------- | --------------------------------------------------------------------------- |
| 0             | UInt8 | Configuration type     | <span class="fixed">`0x1e`</span> (Serialized information notification settings) |
| 1             | UInt8 | Reserved               | `0x00`                                                                      |
| 2             | UInt8 | Notification interval  | `0x01` (10 milliseconds)                                                    |
| 3             | UInt8 | Notification condition | `0x01` (Notifications only when there are changes)                          |
...
Sets the conditions for notifications to be sent. If `0x00` is set, notifications are sent according to the notification interval even if there is no change.
When `0x01` is set, notifications are sent only when there is any change in serialized information. However, even if there are great changes, notifications are not sent at intervals shorter than that of the above notification interval settings.
...
When the connection interval is changed, the cube sends a notification described in [Responses to obtain the actual connection interval value](#responses-to-obtain-the-actual-connection-interval-value).

To obtain the connection interval value at any time, see [Obtaining the actual connection interval value](#obtaining-the-actual-connection-interval-value).  
For how to obtain the result (setting value) of the request to change the connection interval, see [Responses to request to change connection interval](#responses-to-request-to-change-connection-interval).
...
### Request for remote power off

Turn off the cube without pressing its power button by writing the following data.
See [Responses to request for remote power off](#responses-to-request-for-remote-power-off) for how to obtain the response.
The cube cannot be turned on remotely from the power-off state.

| Data location | Type  | Content                  | Example                                                           |
| ------------- | ----- | ------------------------ | ----------------------------------------------------------------- |
| 0             | UInt8 | Configuration type       | <span class="fixed">`0x34`</span> (Request for remote power off)  |
| 1             | UInt8 | Reserved                 | `0x00`                                                            |
| 2             | UInt8 | Time until power off     | `0x0a` (turns off after 10 seconds)                               |

| Time until power off | Meaning                                  |
| -------------------- | ---------------------------------------- |
| `0`                  | Turns off immediately                    |
| `1` through `255`    | Turns off after the specified time (sec) |
...
### Responses to serialized information notification settings

If [Serialized information notification settings](#serialized-information-notification-settings) is written, the following responses are obtained depending on the result.

| Data location | Type  | Content            | Example                                                                                     |
| ------------- | ----- | ------------------ | ------------------------------------------------------------------------------------------- |
| 0             | UInt8 | Configuration type | <span class="fixed">`0x9e`</span> (Responses to serialized information notification settings) |
| 1             | UInt8 | Reserved           | `0x00`                                                                                      |
| 2             | UInt8 | Resulting value    | `0x00` (Set successfully)                                                                   |
...
### Responses to request for remote power off

If [Request for remote power off](#request-for-remote-power-off) is written, the following response is obtained.

| Data location | Type  | Content            | Example                                                                     |
| ------------- | ----- | ------------------ | --------------------------------------------------------------------------- |
| 0             | UInt8 | Configuration type | <span class="fixed">`0xb4`</span> (Responses to request for remote power off) |
| 1             | UInt8 | Reserved           | `0x00`                                                                      |
| 2             | UInt8 | Result value       | `0x00` (Request succeeded)                                                  |
...
| Value other than `0x00` | NG/Request failed    |
