...
以下に示す構成のデータを書き込むことでなめらかな点滅ができます。

| データ位置 | タイプ | 内容                | 例                                                  |
| ---------- | ------ | ------------------- | --------------------------------------------------- |
| 0          | UInt8  | 制御の種類          | <span class="fixed">`0x70`</span>（なめらかな点滅） |
| 1          | UInt8  | 点滅する回数        | `0x00` （回数無制限）                               |
| 2          | UInt8  | 制御するランプの数  | <span class="fixed">`0x01`</span>                   |
| 3          | UInt8  | 制御するランプの ID | <span class="fixed">`0x01`</span>                   |
| 4          | UInt8  | 点滅する周期        | `0x0a` （100 ミリ秒）                               |
| 5          | UInt8  | ランプの Red の値   | `0xFF`                                              |
| 6          | UInt8  | ランプの Green の値 | `0xFF`                                              |
| 7          | UInt8  | ランプの Blue の値  | `0xFF`                                              |
...
| ----------------------- | ---------------------- |
| `0x00`                  | 点滅の回数が無制限     |
| `0x01` 以上 `0xff` 以下 | 指定の回数だけ点滅する |

#### 点滅する周期

なめらかに点灯してなめらかに消灯するまでの周期（時間）を設定します。

| 点滅する周期            | 意味                            |
| ----------------------- | ------------------------------- |
| `0x00` 以上 `0x09` 以下 | 設定無効                        |
| `0x0a` 以上 `0xff` 以下 | 指定の周期 x10 ミリ秒で点滅する |
